...
fieldSpecResampled <- function(directory, outputName = "spectralLibrary_HDW_df"){
tryCatch({
spectralLib_HDW_df<-read.csv(directory)
...
write.csv(alaskaSpeclib_HDW_100nm,"Test_Outputs/2_HDW_Imagery/1_Processing/alaskaSpeclib_HDW_100nm_equal25.csv",row.names = FALSE)
  ...
  message <- paste("WARNING - While processing Field Spec Resampled", directory)
  message <- paste(message, warning, sep = " : ")
  print(message)
}, error = function(error) {
  message <- paste("ERROR - While processing Field Spec Resampled", directory)
